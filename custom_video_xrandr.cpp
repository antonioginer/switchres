...
{
	log_verbose("XRANDR: <%d> (xrandr_timing) creation (%s,%s)\n", m_desktop_output, device_name, param);
	// Copy screen device name and limit size
	if ((strlen(device_name)+1) > 32)
	{
		strncpy(m_device_name, device_name, 31);
		log_error("XRANDR: <%d> (xrandr_timing) [ERROR] the devine name is too long it has been trucated to %s\n", m_desktop_output,m_device_name);
	} else {
...
bool xrandr_timing::init()
{
	...
	if (m_xrandr_handle)
	{
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRAddOutputMode", "XRANDR_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRConfigCurrentConfiguration", "XRANDR_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRCreateMode", "XRANDR_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRDeleteOutputMode", "XRANDR_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRDestroyMode", "XRANDR_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRFreeCrtcInfo", "XRANDR_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRFreeOutputInfo", "XRANDR_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRFreeScreenConfigInfo", "XRANDR_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRFreeScreenResources", "XRANDR_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRGetCrtcInfo", "XRANDR_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRGetOutputInfo", "XRANDR_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRGetScreenInfo", "XRANDR_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRGetScreenResourcesCurrent", "XRANDR_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRQueryVersion", "XRANDR_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRSetCrtcConfig", "XRANDR_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s", m_desktop_output, "XRRSetScreenSize", "XRANDR_LIBRARY");
			return false;
		}
	} else {
		log_error("XRANDR: <%d> (init) [ERROR] missing %s library\n", m_desktop_output, "XRANDR_LIBRARY");
		return false;
	}
	...
	if (m_x11_handle)
	{
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s\n", m_desktop_output, "XGrabServer", "X11_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s\n", m_desktop_output, "XOpenDisplay", "X11_LIBRARY");
			return false;
		}
		...
		if (p_XSync == NULL)
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s\n", m_desktop_output, "XSync", "X11_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s\n", m_desktop_output, "XUngrabServer", "X11_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s\n", m_desktop_output, "XSetErrorHandler", "X11_LIBRARY");
			return false;
		}
		...
		{
			log_error("XRANDR: <%d> (init) [ERROR] missing func %s in %s\n", m_desktop_output, "XGetErrorText", "X11_LIBRARY");
			return false;
		}
	} else {
		log_error("XRANDR: <%d> (init) [ERROR] missing %s library\n", m_desktop_output, "X11_LIBRARY");
		return false;
	}
	...
	log_verbose("XRANDR: <%d> (init) version %d.%d\n", m_desktop_output,major_version,minor_version);
	...
	for (int screen = 0;!detected && screen < ScreenCount(m_pdisplay);screen++)
	{
		...
		for (int o = 0;o < resources->noutput;o++)
		{
			...
			if (output_info->connection == RR_Connected && output_info->crtc && m_desktop_output == -1)
			{
				...
				{
					log_verbose("XRANDR: <%d> (init) name '%s' id %d selected as primary output\n", m_desktop_output, output_info->name, o);
				...
				if (current_rotation & 0xe)
				{
					...
					log_verbose("XRANDR: <%d> (init) desktop rotation is %s\n", m_desktop_output,(current_rotation & 0x2)?"left":((current_rotation & 0x8)?"right":"inverted"));
				}
				output_position++;
			}
...
bool xrandr_timing::set_timing(modeline *mode)
{
	...
	{
		log_verbose("XRANDR: <%d> (set_timing) mode [%04lx] is already active\n", m_desktop_output,pxmode->id);
		XRRFreeCrtcInfo(crtc_info);
		XRRFreeOutputInfo(output_info);
		XRRFreeScreenResources(resources);
		return true;
	...
	log_verbose("XRANDR: <%d> (set_timing) switching mode [%04lx] %ux%u+%d+%d --> [%04lx] %ux%u+%d+%d\n", m_desktop_output, crtc_info->mode, crtc_info->width, crtc_info->height, crtc_info->x, crtc_info->y, pxmode->id, pxmode->width, pxmode->height, crtc_info->x, crtc_info->y);
	...
	for (int c = 0;c < resources->ncrtc;c++)
	{
		...
		//log_verbose("XRANDR: <%d> (set_timing) <debug> disable crtc %d mode id %04lx time %ld/%ld\n", m_desktop_output, c, crtc_info2->mode, crtc_info2->timestamp, global_crtc[c].timestamp);
	...
	for (int c = 0;c < resources->ncrtc;c++)
	{
		...
		if ( crtc_info2->mode != 0 && crtc_info2->timestamp == 1)
		{
			...
			if (m_xerrors & 0x10)
			{
				log_error("XRANDR: <%d> (set_timing) [ERROR] in %s\n", m_desktop_output,"XRRSetCrtcConfig");
				m_xerrors &= 0xEF;
			}
		}
	}
	...
	for (int m = 0;m < resources->nmode && crtc_info->mode;m++)
	{
		...
		if (pxmode2->id == crtc_info->mode)
			log_verbose("XRANDR: <%d> (set_timing) active mode [%04lx] name %s clock %6.6fMHz %ux%u+%d+%d\n", m_desktop_output, pxmode2->id, pxmode2->name, (double)pxmode2->dotClock / 1000000.0, pxmode->width, pxmode->height, crtc_info->x, crtc_info->y);
	}
...
bool xrandr_timing::delete_mode(modeline *mode)
{
	...
	for (int m = 0;m < resources->nmode && mode->platform_data != 0;m++)
	{
		if (mode->platform_data == resources->modes[m].id)
		{
			...
			if (m_xerrors & m_xerrors_flag)
			{
				log_error("XRANDR: <%d> (delete_mode) [ERROR] in %s\n", m_desktop_output,"XRRDestroyMode");
				total_xerrors++;
			}
